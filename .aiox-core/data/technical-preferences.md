...
This project is an existing static/vanilla JavaScript site. Agents should treat the
runtime stack as HTML, CSS, and vanilla JavaScript with no build step unless the
project explicitly adds one later.
...
techPreset:
  active: none # Static/vanilla JavaScript project
```
...
|----------|------------|-------|
| Frontend Framework | Vanilla JavaScript | Existing custom SPA/micro-framework |
| Styling | Plain CSS | Existing CSS assets under `src/assets/css/` |
| Build Process | None | Static files served directly |
| Runtime | Browser | No server-side runtime in this app |
